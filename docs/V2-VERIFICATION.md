...
最近一次测试报告包含 `23` 个 test suite、`65` 个测试，`0` failure、`0` error、`0` skipped。core 和 integrated 构建均已到达 `BUILD SUCCESSFUL`。编译仅有 Forge/Minecraft 弃用 API 警告，没有测试失败。模组元数据版本为合法语义版本 `2.0.0`，文件名保留 `v2`。当前成品校验值：

- core：`28BFBF54944DCE12F7026D2A1B594C25FE034590824B2D912F9269A8A3CA4A2A`
- integrated：`DC86208353A7D30DF5215327ACF348851653D35D1DDFD023C80B00E19975180A`
...
- [x] 用户提供的 8 张 18×18 状态图标已替换 HUD 占位形状，四肢黑化按具体部位分行显示；背景透明度与 MobEffect 资源副本已通过自动测试。

### 可选兼容

- [x] EnhancedVisuals、Camera Overhaul、Explosion Overhaul 版本选择策略和未知版本安全降级。
- [x] EnhancedVisuals 因创造/旁观配置拒绝绘制时归还屏幕通道，内置 fallback 不会被静默吞掉。
...
3. 表现与无障碍：视觉、音频、减少动态、色散、心跳、高对比状态卡、总强度、镜头强度和独立耳鸣音量。

命令树已包含 `/healthfx client ...`、兼容别名 `/healthfx server ...` 与独立服务端根 `/healthfx_server ...`。服务端写操作要求权限等级 2；F8 使用独立根避免 Forge 客户端同名命令拦截。命令树结构测试确认 repair、analgesia、regeneration 和 head_redirect 均为服务端根的直接子命令。完整语法见 [`../README.md`](../README.md)。
